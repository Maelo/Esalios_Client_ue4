...
{
	TSharedPtr<FJsonObject> mapContentNode = jsonObject->GetObjectField("mapContent");

	//The person "object" that is retrieved from the given json file
	TArray <TSharedPtr<FJsonValue>> sectorArray = mapContentNode->GetArrayField("sectorList");

	FString mapName = mapContentNode->GetStringField("name");

	TSharedPtr<FJsonObject> mapSizeJson = mapContentNode->GetObjectField("size");

	FVector2D size;
	size.X = mapSizeJson->GetNumberField("x");
	size.Y = mapSizeJson->GetNumberField("y");

	EFTAMap* map = new EFTAMap(mapName, size);
